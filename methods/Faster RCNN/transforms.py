...
def get_transform(train):
    ...
    #transforms.append(Normalize((98.965, 98.965, 98.965),(50.584, 50.584, 50.584)))
    if train:
        transforms.append(RandomHorizontalFlip(0.5))
        transforms.append(HSVJitter(0.4, 0.5, 0.7, 0.05))
        #transforms.append(RandomAffine(0, scale=(0.5, 1.0)))
...
class RandomAffine(T.RandomAffine):
    def forward(self, image: Tensor, target: Optional[Dict[str, Tensor]] = None) -> Tuple[Tensor, Optional[Dict[str, Tensor]]]:
        if self.scale != None:
            scalefactor = (self.scale[0] - self.scale[1]) * torch.rand(1) + self.scale[1]
            image = F.resize(image, [int(image.shape[-1] * scalefactor), int(image.shape[-2] * scalefactor)])
            if target is not None:
                target["boxes"][:, [0,1,2,3]] = scalefactor * target["boxes"][:, [0,1,2,3]]
        return image, target
...
class HSVJitter(T.ColorJitter):
    def forward(self, img: Tensor, target: Optional[Dict[str, Tensor]] = None) -> Tuple[Tensor, Optional[Dict[str, Tensor]]]:
